...
RDKIT_GIT_URL=${2:-"https://github.com/varunbachalli/rdkit-export-testing.git"}
echo "Building distribution files for release $RDKIT_BRANCH from repo $RDKIT_GIT_URL"
DOCKER_BUILDKIT=1 docker build --progress=plain --no-cache -f docker/Dockerfile --build-arg RDKIT_BRANCH=$RDKIT_BRANCH --build-arg RDKIT_GIT_URL=$RDKIT_GIT_URL -o $MINIMALLIB_OUTPUT_PATH .
